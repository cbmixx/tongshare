...
/*
 * SpaceFlyer changed this code to handle Chinese Pinyin.
 * So make sure to to include pinyin.js beefore this.
*/
...
dhtmlXCombo.prototype.getIndexByValue = function(val){
    for(var i=0;i<this.optionsArr.length;i++)if(this.optionsArr[i].value == val)return i;return -1;
};
...
dhtmlXCombo.prototype.getSelectedIndex = function(){
    for(var i=0;i<this.optionsArr.length;i++)if(this.optionsArr[i] == this._selOption)return i;return -1;
};
...

{
    ...
    for(var i=0;i<this.optionsArr.length;i++){
        var pinyins = makePy(this.optionsArr[i].text) //Make sure to include pinyin.js beefore this
        var z = false
        for(var j=0; j<pinyins.size(); j++){
            z |= filter.test(this.optionsArr[i].text + pinyins[j]);
        }
        this.filterAny|=z;
